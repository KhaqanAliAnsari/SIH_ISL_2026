...
export const VideoPanel: React.FC<VideoPanelProps> = ({
  ...
  useEffect(() => {
    ...
    const runDetection = () => {
      ...
      // Throttled AI extraction (decoupled from 60fps render rate)
      if (now - lastExtractionTimeRef.current >= EXTRACTION_INTERVAL_MS) {
        const t0 = performance.now();

        // 1. Detect pose + hands (pose throttled to 1-in-4 cycles internally)
        ...
        if (detected) {
          ...
          // Evaluate DTW every 2nd extraction cycle to cut CPU load by 50%
      ...
      // 4. Render at full rate using last known data (deduplicated)
      if (lastResultRef.current && handVisibleRef.current) {
        if (lastExtractionTimeRef.current !== lastDrawnExtractionTimeRef.current) {
          ...
          actualDrawsRef.current++;
          lastDrawnExtractionTimeRef.current = lastExtractionTimeRef.current;

          if (Math.random() < 0.05) {
            console.log(`[Profiler] drawLandmarks time: ${(tDrawEnd - tDrawStart).toFixed(2)}ms`);
          }
        } else {
          skippedDrawsRef.current++;
        }
      } else if (!showMesh) {
        if (lastDrawnExtractionTimeRef.current !== -1) {
          ctx.clearRect(0, 0, canvas.width, canvas.height);
          lastDrawnExtractionTimeRef.current = -1;
        }
      }
      ...
      if (now - lastFpsTimeRef.current >= 1000) {
        console.log(`[Profiler] FPS: ${Math.round((fpsFrameCountRef.current * 1000) / (now - lastFpsTimeRef.current))} | Skipped draws: ${skippedDrawsRef.current} | Actual draws: ${actualDrawsRef.current}`);
        fpsFrameCountRef.current = 0;
        skippedDrawsRef.current = 0;
        actualDrawsRef.current = 0;
