# Graph Report - SIH_ISL_2026  (2026-08-22)

## Corpus Check
- 267 files · ~477,581 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3598 nodes · 6756 edges · 256 communities (197 shown, 59 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 118 edges (avg confidence: 0.69)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f2d20f9c`
...
## Communities (256 total, 59 thin omitted)

### Community 0 - "dependencies"
Cohesion: 0.13
Nodes (15): express, @google/genai, @mediapipe/tasks-vision, dependencies, express, @google/genai, @mediapipe/tasks-vision, react-dom (+7 more)
...
Cohesion: 0.10
Nodes (33): TemplateRecorderModal(), TemplateRecorderModalProps, KEY_LANDMARKS, VideoPanel(), cachedTemplateNames, clearBuffer(), encodeNpy(), FEATURE_DIM (+25 more)

### Community 16 - "_read_text"
Cohesion: 0.02
Nodes (175): _get_c_func_name(), _import_c(), _import_lua(), _import_swift(), Emit module-level ``imports`` edges and report the imported modules. A Swift…, Get the name from a node using config.name_field, falling back to child types., Recursively unwrap declarator to find the innermost identifier (C)., Extract require('module') from Lua variable_declaration nodes. (+167 more)

### Community 17 - "extract.py"
Cohesion: 0.05
Nodes (114): _augment_js_reexport_edges(), extract_vue(), Deterministic structural extraction from source code using tree-sitter. Outputs…, Collapse whitespace and truncate ``text`` to ``width`` chars for a rationale…, Extract imports, symbols, and type refs from a ``.vue`` SFC. Masks the…, Compatibility wrapper for the JS/TS symbol-resolution post-pass., _shorten_rationale_label(), LanguageConfig (+106 more)
...
### Community 20 - "cli.py"
Cohesion: 0.05
Nodes (80): distinct_repo_tags(), edge_datas(), prefix_graph_for_global(), prune_repo_from_graph(), Return a copy of G with all node IDs prefixed with repo_tag::. Labels are…, Return a unique, human-meaningful repo tag per input graph for merge-graphs.…, Remove all nodes tagged with repo_tag from G in-place. Returns count removed., Return every edge attribute dict for (u, v); always a list. (+72 more)
...
Cohesion: 0.09
Nodes (37): attach_hyperedges(), backup_if_protected(), _cap_filename(), _cypher_escape(), _cypher_label(), _dedup_node_filenames(), _git_head(), _obsidian_safe_stem() (+29 more)

### Community 23 - "Path"
Cohesion: 0.04
Nodes (86): _augment_cpp_string_tests(), _canonicalize_csharp_namespace_nodes(), _check_tree_sitter_version(), extract(), extract_cpp(), extract_csharp(), extract_csproj(), extract_groovy() (+78 more)
...
Cohesion: 0.08
Nodes (49): _bfs(), _complete_induced_edges(), _compute_idf(), _cut_lines_to_budget(), _dfs(), _display_graph_path(), _filter_graph_by_context(), _find_node() (+41 more)

### Community 26 - "_extract_generic"
Cohesion: 0.04
Nodes (55): _emit_rescued_import(), extract_astro(), extract_c(), extract_java(), extract_js(), _extract_js_rationale(), extract_kotlin(), extract_lua() (+47 more)
...
Cohesion: 0.21
Nodes (10): dtwCurrRow, dtwDistance(), dtwPrevRow, euclideanDistanceSq(), GestureTemplateClass, GestureVariation, getRingBufferSnapshot(), matchGesture() (+2 more)
...
### Community 34 - "prs.py"
Cohesion: 0.20
Nodes (26): _default_model_for_backend(), Return configured model override or backend default model., bold(), _c(), _ci_icon(), _classify(), cmd_prs(), cyan() (+18 more)
...
Cohesion: 0.03
Nodes (124): extract_lazarus_package(), _import_java(), _import_php(), _import_scala(), Extract package metadata from Lazarus .lpk package files (XML format). .lpk is…, extract_apex(), Path, Apex extractor. Moved verbatim from graphify/extract.py. (+116 more)
...
Cohesion: 0.12
Nodes (26): Read just this slice's characters from its parent file., read_slice_text(), _bind_node_evidence(), _build_image_refs(), _dispatched_source_text(), _estimate_file_tokens(), extract_files_direct(), _file_to_text() (+18 more)

### Community 41 - "mem0-memory/package.json"
Cohesion: 0.18
Nodes (10): dependencies, mem0ai, @modelcontextprotocol/sdk, description, mem0ai, main, name, type (+2 more)
...
### Community 46 - "_resolve_js_import_target"
Cohesion: 0.18
...
Cohesion: 0.13
Nodes (16): _anthropic_response_text(), _bedrock_inference_config(), _bedrock_response_text(), _call_llm(), _model_requires_default_temperature(), Return the first Anthropic content block that carries text. Current Claude…, Return the first Converse content block that carries text. Converse returns…, Send a plain-text prompt to `backend` and return the model's text reply. When… (+8 more)
...
### Community 70 - "llm.py"
Cohesion: 0.10
Nodes (27): _backend_supports_vision(), _community_label_lines(), _custom_providers_path(), generate_community_labels(), _get_tokenizer(), _label_batch_with_retry(), label_communities(), _load_custom_providers() (+19 more)

### Community 71 - "build.py"
Cohesion: 0.04
Nodes (87): _estimate_tokens(), Graph, _query_subgraph_tokens(), Token-reduction benchmark - measures how much context graphify saves vs naive…, Run BFS from best-matching nodes and return estimated tokens in the subgraph…, Measure token reduction: corpus tokens vs graphify query tokens. Args:…, run_benchmark(), _abs_identity() (+79 more)
...
### Community 79 - "_call_claude_cli"
Cohesion: 0.24
Nodes (10): _call_claude_cli(), _claude_cli_envelope(), _claude_cli_error(), _claude_cli_supports_json_schema(), _no_window_kwargs(), Parse the JSON returned by `claude -p --output-format json`. Older Claude Code…, Return the CLI's own error text when the envelope flags `is_error`. `claude -p`…, Return True if this Claude Code CLI accepts ``--json-schema``. Structured… (+2 more)
...
### Community 81 - "_remove_marker_section"
Cohesion: 0.25
...
### Community 82 - "introspect_postgres"
Cohesion: 0.50
...
Cohesion: 0.50
Nodes (3): memory, server, transport

### Community 86 - "print_benchmark"
Cohesion: 0.40
Nodes (6): _hr(), print_benchmark(), Print a human-readable benchmark report., Return unicode_char if stdout can encode it, else ascii_fallback. Windows…, Horizontal rule that survives non-UTF-8 stdout (e.g. Windows cp1252 console)., _safe()

### Community 87 - "PhrasedSentence"
Cohesion: 0.67
...
### Community 166 - "_get_backend_api_key"
Cohesion: 0.33
Nodes (6): _backend_env_keys(), _format_backend_env_keys(), _get_backend_api_key(), Return accepted API-key environment variables for a backend., Return the first configured API key for backend, or an empty string., Return user-facing accepted API-key variable names.
...
## Knowledge Gaps
- **806 isolated node(s):** `memory`, `server`, `transport`, `name`, `version` (+801 more)
...
- **Why does `dispatch_command()` connect `cli.py` to `introspect_cargo`, `reflect.py`, `watch.py`, `cache.py`, `export.py`, `Path`, `hooks.py`, `serve.py`, `analyze.py`, `prs.py`, `write_callflow_html`, `_get_backend_api_key`, `_call_bedrock`, `diagnostics.py`, `ingest.py`, `llm.py`, `build.py`, `detect`, `file_slice.py`, `cluster.py`, `__main__.py`, `introspect_postgres`, `print_benchmark`, `affected.py`, `save_manifest`, `semantic_cleanup.py`, `html.py`, `wiki.py`, `_build_server`, `tree_html.py`, `_load_graph`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Why does `_file_stem()` connect `_make_id` to `sanitize_metadata`, `build.py`, `mcp_ingest.py`, `verilog.py`, `_resolve_js_import_target`, `_read_text`, `extract.py`, `Path`, `_extract_generic`, `symbol_resolution.py`?**
  _High betweenness centrality (0.024) - this node is a cross-community bridge._
- **Why does `extract()` connect `Path` to `_make_id`, `extract.py`, `resolver_registry.py`, `watch.py`, `cli.py`, `cache.py`, `symbol_resolution.py`, `CsharpNameResolver`?**
...
- **What connects `memory`, `server`, `transport` to the rest of the system?**
