# Graph Report - SIH_ISL_2026  (2026-09-09)

## Corpus Check
- 293 files · ~608,043 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3927 nodes · 7302 edges · 306 communities (205 shown, 101 thin omitted)
...
## Communities (306 total, 101 thin omitted)

### Community 0 - "_read_text"
Cohesion: 0.02
Nodes (173): _get_c_func_name(), _import_js(), _import_swift(), Emit module-level ``imports`` edges and report the imported modules. A Swift…, Get the name from a node using config.name_field, falling back to child types., Recursively unwrap declarator to find the innermost identifier (C)., _resolve_name(), _read_text() (+165 more)

### Community 1 - "cli.py"
Cohesion: 0.05
Nodes (75): print_benchmark(), Print a human-readable benchmark report., Measure token reduction: corpus tokens vs graphify query tokens. Args:…, run_benchmark(), disambiguate_file_labels_in_nodes(), distinct_repo_tags(), prefix_graph_for_global(), prune_repo_from_graph() (+67 more)

### Community 2 - "watch.py"
Cohesion: 0.06
Nodes (61): dedupe_edges(), dedupe_nodes(), Collapse nodes sharing an ``id``, last-writer-wins on attributes. Mirrors what…, Collapse exact parallel edges by ``(source, target, relation)``, keeping the…, community_member_sigs(), Per-community membership fingerprints: ``{cid: sha256(sorted member ids)}``.…, Remap community IDs to maximize overlap with a previous assignment. Uses greedy…, remap_communities_to_previous() (+53 more)

### Community 3 - "_make_id"
Cohesion: 0.03
Nodes (109): _import_java(), _import_php(), _import_python(), _import_scala(), extract_apex(), Path, Apex extractor. Moved verbatim from graphify/extract.py., Extract classes, interfaces, enums, methods, and Salesforce constructs from… (+101 more)

### Community 4 - "extract.py"
Cohesion: 0.04
Nodes (120): _augment_js_reexport_edges(), extract_astro(), extract_vue(), _import_lua(), Deterministic structural extraction from source code using tree-sitter. Outputs…, Collapse whitespace and truncate ``text`` to ``width`` chars for a rationale…, Extract imports from .astro files: frontmatter (TS) + template regex fallback.…, Extract imports, symbols, and type refs from a ``.vue`` SFC. Masks the… (+112 more)

### Community 5 - "Path"
Cohesion: 0.03
Nodes (103): _augment_cpp_string_tests(), _canonicalize_csharp_namespace_nodes(), _check_tree_sitter_version(), _emit_rescued_import(), extract(), extract_c(), extract_cpp(), extract_csharp() (+95 more)
...
Cohesion: 0.06
Nodes (66): _abs_identity(), build(), build_from_json(), build_merge(), _build_prune_sets(), _coerce_hyperedge_member_refs(), _coerce_id(), _coerce_non_string_ids() (+58 more)

### Community 11 - "serve.py"
Cohesion: 0.08
Nodes (50): _bfs(), _complete_induced_edges(), _compute_idf(), _cut_lines_to_budget(), _dfs(), _display_graph_path(), _filter_graph_by_context(), _find_node() (+42 more)
...
### Community 17 - "dtwEngine.ts"
Cohesion: 0.15
Nodes (14): TemplateRecorderModal(), TemplateRecorderModalProps, cachedTemplateNames, clearBuffer(), encodeNpy(), FEATURE_DIM, getBufferFill(), getTemplateCount() (+6 more)

### Community 18 - "export.py"
Cohesion: 0.07
Nodes (45): _node_community_map(), Invert communities dict: node_id -> community_id., attach_hyperedges(), _cap_filename(), _cypher_escape(), _cypher_label(), _dedup_node_filenames(), existing_graph_node_count() (+37 more)
...
Cohesion: 0.09
Nodes (32): extract_csproj(), extract_lazarus_package(), extract_slnx(), _project_xml_is_safe(), Reject XML that declares DTDs or entities. Stdlib ``xml.etree.ElementTree``…, Extract package metadata from Lazarus .lpk package files (XML format). .lpk is…, Extract projects and inter-project dependencies from a .slnx file. .slnx is the…, Extract packages, project refs, and target framework from a… (+24 more)
...
Cohesion: 0.11
Nodes (19): @capacitor/android, @capacitor/camera, @capacitor/cli, @capacitor/haptics, @capacitor/status-bar, dependencies, @capacitor/android, @capacitor/camera (+11 more)
...
### Community 61 - "VideoPanel.tsx"
Cohesion: 0.15
Nodes (27): KEY_LANDMARKS, VideoPanel(), getTemplateNames(), loadTemplates(), matchGesture(), pushFrame(), closeMainThreadLandmarker(), detectMainThread() (+19 more)
...
### Community 68 - "semantic_cleanup.py"
Cohesion: 0.19
...
Cohesion: 0.12
Nodes (24): _estimate_tokens(), _hr(), Graph, _query_subgraph_tokens(), Token-reduction benchmark - measures how much context graphify saves vs naive…, Return unicode_char if stdout can encode it, else ascii_fallback. Windows…, Horizontal rule that survives non-UTF-8 stdout (e.g. Windows cp1252 console)., Run BFS from best-matching nodes and return estimated tokens in the subgraph… (+16 more)
...
### Community 84 - "_GraphContextCache"
Cohesion: 0.25
...
### Community 263 - "extract_xaml"
Cohesion: 0.22
Nodes (13): extract_xaml(), Extract WPF/XAML structure, bindings, x:Class, and event handler references., _xaml_binding_refs(), _xaml_communitytoolkit_members(), _xaml_explicit_viewmodel_names(), _xaml_inferred_viewmodel_names(), _xaml_markup_extension(), _xaml_pascal_name() (+5 more)

### Community 267 - "tree_html.py"
Cohesion: 0.38
...
### Community 270 - "_looks_like_context_exceeded"
Cohesion: 0.40
...
- **Why does `dispatch_command()` connect `cli.py` to `watch.py`, `Path`, `cache.py`, `reflect.py`, `analyze.py`, `build.py`, `serve.py`, `detect`, `tree_html.py`, `hooks.py`, `export.py`, `extract_files_direct`, `write_callflow_html`, `__main__.py`, `_ImageRef`, `prs.py`, `html.py`, `diagnostics.py`, `ingest.py`, `_run_hook_guard`, `affected.py`, `cluster.py`, `save_manifest`, `llm.py`, `semantic_cleanup.py`, `edge_data`, `_build_server`, `introspect_cargo`, `introspect_postgres`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Why does `extract()` connect `Path` to `cli.py`, `watch.py`, `_make_id`, `extract.py`, `CsharpNameResolver`, `cache.py`, `symbol_resolution.py`, `resolver_registry.py`?**
  _High betweenness centrality (0.024) - this node is a cross-community bridge._
- **Why does `_file_stem()` connect `_make_id` to `_read_text`, `extract.py`, `Path`, `extract_xaml`, `sanitize_metadata`, `build.py`, `symbol_resolution.py`, `objc.py`, `verilog.py`, `mcp_ingest.py`, `pascal.py`?**
...
- **Should `_read_text` be split into smaller, more focused modules?**
  _Cohesion score 0.01793103448275862 - nodes in this community are weakly interconnected._